    ...
    <div class="_body">
      <div class="wrapper" style="padding: 0rem;">
        ...
        <div class="row fx-white index-content">
          ...
          <div class="col-xs-4 fc-color-orange _captcha_button" v-on:click="captcha()"  :style="this.stop==true?'border: 0.1rem solid #dadada;color:#dadada;float: right; padding:0rem;':'float: right; padding:0rem;'">
          <span style="font-size:1.2rem">{{validText}}
          </span></div>
        </div>
      </div>
    </div>
  ...
  export default{
    ...
    mounted(){
      ...
      if(api.getQueryString("code")){
        ...
        localStorage.setItem("wx.user.code",this.params.code);
        localStorage.setItem("wx.user.wechat.code",this.params.wechatCode);
      }
    },
    beforeCreate() {
