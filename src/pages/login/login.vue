<template>
    <div>
        <div class="ljb-pop">
            ...
            <div class="content">
                <div class="phone"><i class="iconfont">&#xe63e;</i><input type="text" placeholder="请输入手机号码" v-model="params.mobilePhone"></div>
                <div class="code"><i class="iconfont">&#xe701;</i><input type="text" placeholder="验证码"  v-model="params.captchaCode" ><a  v-on:click="captcha()" >{{validText}}</a></div>
                <div class="btn" style="display: block"><a  style="height: 4rem;line-height: 4rem;" @click="userLogin">{{loginText}}</a></div>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            successWatcher(val, oldVal) {
                ...
                console.info("WX_USER_PHONE="+localStorage.getItem(WX_USER_PHONE));
                ...
                console.info("登录成功跳转地址="+redirectUrl);
                Indicator.close();
...
//              this.onLogin = true;
            ...
            captcha() {
                ...
                //埋点
                _track._trackEvent("JCDZ_WX", "获取登陆验证码", this.params.mobilePhone, "");
                //调用store中的accountLoginSubmit方法
