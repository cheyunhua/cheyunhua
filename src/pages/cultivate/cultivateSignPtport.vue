	...
	<div class="cultivate-sign">
		...
		<div class="personalInfo-form" v-if="fillForm">
			...
			<div class="personalInfo-item">
				...
				<div class="personalInfo-item-input">
					<input type="tel" placeholder="请输入您的手机号" v-model="phone" ref="input" @focus="closeSelectAreaAgent()" :readonly="this.existSignText.length>0?'true':false" v-bind:class="this.existSignText.length>0?'noselect':''" />
				</div>
			</div>
			<div class="personalInfo-item">
	...
	export default {
		...
		methods: {
			...
			sureSign() {
				...
				info.wechatId = this.wechatId;
				info.trainId = this.sceneId;
				info.userName = this.name;
				//				info.companyId = this.companyId;
				info.mobilePhone = this.phone;
				for(let i = 0; i < singInfoLength; i++) {
					...
					cusList.push({ "taxIdentifNumber": this.nsrsbh[i], "cusName": this.company[i] });
					//					singInfo.push(info);
				}
				info.cusList = cusList;
				params = info;
				api.saveTrainSign(params).then((response) => {
					...
					} else if(response.data.head.errorCode == '04030041') {
						this.$message({
							message: '今天已经签到过了',
							type: 'error',
							customClass: 'messageAl'
			...
			queryTrainSignByOpenid() {
				...
				params.trainId = this.sceneId;
				api.queryTrainSignByOpenid(params).then((response) => {
					if(response.data.head.errorCode == '0') {
						if(response.data.body.length > 0) {
							Indicator.close();
							this.fillForm = false;
							this.name = response.data.body[0].userName;
							this.phone = response.data.body[0].mobilePhone;
							for(let i = 0; i < response.data.body.length; i++) {
								this.addlists.push({ company: response.data.body[i].cusName, nsrsbh: response.data.body[i].taxIdentificationNumber });
							}
							/*	this.theme = response.data.body.trainInfo.trainTheme;
								this.areaName = response.data.body.trainInfo.areaName;*/
							this.queryTrainInfoById();
							this.buttonText = '已签到';
							this.existSignText = '亲！今天签到过了(∩_∩)'
						} else {
							this.fillForm = true;
			...
			queryTrainInfoById() {
				let params = {};
				params.trainId = this.sceneId;
				api.queryTrainInfoById(params).then((response) => {
					if(response.data.head.errorCode == '0') {
						this.theme = response.data.body.trainName;
						this.areaName = response.data.body.areaFullName;
					}
				}).catch((response) => {
					Toast('哎呀！网络出了点问题，请稍后重试！');
				});
			...
			sendTrainInfo() {
				...
				api.sendTrainInfo(params).then((response) => {
					if(response.data.head.errorCode == '0') {
...
//			this.appId = this.$route.query.appId != undefined ? this.$route.query.appId : '';
			if(this.sceneId) {
				this.queryTrainSignByOpenid();
			}
			this.addlistsValues();
		},
	}
